...
async function doCharAction({ action, guildId, userId, charId, payload, guildOwnerId = null }) {
	...
	const isGM = guildOwnerId === userId;
	if (!isOwner && !isGM) throw new Error('You do not own that character.');
