...
function renderPlaybookEmbed(record, section) {
	...
	case 'overview': {
		embed = new EmbedBuilder()
			...
			.addFields(
				{ name: 'Starting Moves', value: pb.creationRules?.intro || '—', inline: false },
			);
		break;
	}
	case 'background': {
		...
		embed = new EmbedBuilder()
			.setTitle(`${record.name} — Background: ${chosenBackground || '—'}`)
	...
	case 'instinct': {
		const text = chosenInstinct && pb.instincts?.[chosenInstinct]
			...
			: '`(not recorded for this character)`';
		embed = new EmbedBuilder()
			.setTitle(`${record.name} — Instinct: ${chosenInstinct || '—'}`)
			.setDescription(text.slice(0, 4096));
		break;
	}
	case 'moves': {
	...
	embed.setFooter({ text: `${pb.name} — ${section}` });
