...
function renderPlaybookEmbed(record) {
	...
	const chosenBackground = record.choices?.background || null;
	const chosenInstinct = record.choices?.instinct || null;
	const chosenMoves = Array.isArray(record.choices?.chosen_moves)
		? record.choices.chosen_moves
		: [];

	const die = pb.creationRules?.die || '—';
	const bgText = chosenBackground
		? `**${chosenBackground}**\n${pb.backgrounds?.[chosenBackground]?.text || ''}`
		: Object.keys(pb.backgrounds).map(b => `• ${b}`).join('\n');

	const instinctText = chosenInstinct
		? `**${chosenInstinct}** — ${pb.instincts?.[chosenInstinct] || ''}`
		: Object.entries(pb.instincts).map(([k, v]) => `• **${k}** — ${v}`).join('\n');

	const moveLines = chosenMoves.length
		? chosenMoves.map(m => `• ${m}${pb.allMoves?.[m]?.text ? ` — ${pb.allMoves[m].text.split('\n')[0]}` : ''}`).join('\n')
		: '—';

	const insertLines = Array.isArray(pb.inserts) && pb.inserts.length
		? pb.inserts.map(i => `• ${i}`).join('\n')
		: '—';
	...
	const embed = new EmbedBuilder()
		...
		.addFields(
			...
			{ name: 'Background', value: bgText.slice(0, 1024) || '—', inline: false },
			{ name: 'Instinct', value: instinctText.slice(0, 1024) || '—', inline: false },
			{ name: 'Moves', value: moveLines.slice(0, 1024) || '—', inline: false },
			{ name: 'Inserts', value: insertLines.slice(0, 1024) || '—', inline: false },
		)
...
---`, starred: false },
	...
	creationRules: { intro: '*You start with **Ambush** OR **Skill at Arms**; **Danger Sense** OR **Perceptive**; and 1 of your choice.*', die: 'd6', maxHP: 16, orGroups: [{ label: 'Weapon Training', options: ['Ambush', 'Skill at Arms'] }, { label: 'Natural Instincts', options: ['Danger Sense', 'Perceptive'] }], startingPickCount: 1 },
};
playbooks.Fox = Fox;
...
---`, starred: false },
	...
	creationRules: { intro: '*You start with **Dangerous**, **Hard to Kill**, and either **Armored** OR **Uncanny Reflexes**.*', die: 'd6', maxHP: 20, orGroups: [{ label: 'Defensive Style', options: ['Armored', 'Uncanny Reflexes'] }], startingPickCount: 0 },
};
