...
const { splitIntoChunks, tagPagination } = require('./embed_pager');
...
function renderPlaybookEmbed(record, section) {
	...
	case 'background': {
		...
		embeds = chunks.map((chunk, i) => {
			...
			tagPagination(e, `${pb.name} — ${sectionTitle}`, i, chunks.length);
			return e;
		});
		break;
	}
	...
	case 'moves': {
		...
		if (chosenMoves.length) {
			moveList = chosenMoves.map(m => {
				const md = pb.allMoves?.[m];
				const text = md && md.text ? md.text.split('\n---')[0] : '';
				return `**${m}**\n${text || '—'}`;
			}).join('\n\n');
		}
		const chunks = splitIntoChunks(moveList);
		embeds = chunks.map((chunk, i) => {
			const e = new EmbedBuilder()
				.setTitle(`${sectionTitle} — ${record.name}`)
				.setDescription(chunk);
	...
	// Attach timestamp to every page.
	embeds.forEach(e => e.setTimestamp(new Date()));
	return embeds;
}
...
---`, starred: false },
	...
	creationRules: { intro: '*You start with either **Weapon Training** (Ambush + Skill at Arms) or **Natural Instincts** (Danger Sense + Perceptive), plus 1 of your choice.*', die: 'd8', maxHP: 16, orGroups: [{ label: 'Choose your specialty', options: [{ name: 'Weapon Training', grants: ['Ambush', 'Skill at Arms'], desc: 'Ambush + Skill at Arms' }, { name: 'Natural Instincts', grants: ['Danger Sense', 'Perceptive'], desc: 'Danger Sense + Perceptive' }] }], startingPickCount: 1,
		possessions: {
			pickCount: 2,
			options: [
...
---`, starred: false },
	...
	creationRules: { intro: '*You start with **Dangerous**, **Hard to Kill**, and either **Armored** OR **Uncanny Reflexes**.*', die: 'd10', maxHP: 20, orGroups: [{ label: 'Defensive Style', options: [{ name: 'Armored', grants: ['Armored'], desc: 'Uncanny reflexes, thick hide, or masterwork plate' }, { name: 'Uncanny Reflexes', grants: ['Uncanny Reflexes'], desc: 'Light-footed and impossible to pin down' }] }], startingPickCount: 0,
		possessions: {
			pickCount: 2,
			options: [
				{ name: 'Distillery', desc: 'Fine whisky (uses grant advantage to Persuade), copper tubes, malt, firkins, stills, barrels.' },
...
---`, starred: false },
	...
	creationRules: { intro: '*You start with **Consecrated Flame** and **Invoke the Sun God**, plus 1 more of your choice.*', die: 'd4', maxHP: 18, startingPickCount: 1,
		possessions: {
			pickCount: 2,
			options: [
...
---`, starred: false },
	...
	creationRules: { intro: '*You start with **Crew**, **Logistics**, any moves from your Background, and 1 move of your choice.*', die: 'd8', maxHP: 20, startingPickCount: 1,
		possessions: {
			pickCount: 2,
			options: [
...
---`, starred: false },
	...
	creationRules: { intro: '*You start with **Home on the Range**, any moves from your Background, plus 1 of your choice.*', die: 'd8', maxHP: 18, startingPickCount: 1,
		possessions: {
			pickCount: 2,
