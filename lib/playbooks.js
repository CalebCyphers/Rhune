 ...
 */
function getMovePreview(moveName, pb) {
	const move = pb.allMoves?.[moveName];
	if (!move?.text) return '';
...
function renderPlaybookEmbed(record) {
	...
	const moveLines = chosenMoves.length
		? chosenMoves.map(m => `• ${m}${getMovePreview(m, pb)}`).join('\n')
		: '—';
...
module.exports = { lookupPlaybook, renderPlaybookEmbed, getAllMoves, parseChoices, getCharacterMoveList };
