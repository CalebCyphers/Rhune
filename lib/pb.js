...
const MAX_ADMIN_REAUTH = 2;

// PocketBase returns 403 "Only admins can perform this action." when a request
// hits an ADMIN-ONLY collection (rules = null) WITHOUT a valid (non-expired)
// admin token. On this old PB (v0.17-style), an *expired* admin token surfaces
// as this 403 rather than a 401 — so we must treat it as a re-auth signal.
// (Real permission denials for anonymous access to admin-only collections are
// indistinguishable from an expired-token case, so the safe move is re-auth+retry
// a bounded number of times, then surface the original error.)
const ADMIN_403_REAUTH = true;
...
function isAdminRejected(err) {
	...
	return raw.includes('only admins can perform this action')
		|| raw.includes('admin authorization token')
		|| raw.includes('requires valid admin');
}
 ...
 *  - 403 "Only admins..." (admin-only collection + missing/expired admin token)
 *    -> forceReauth() and retry a bounded number of times.
 ...
 *  - A real, non-expiry 403 (e.g. an anonymous user genuinely lacking rights) will
 *    still be returned to the caller after the bounded re-auth retries are exhausted.
 *
...
async function runPb(fn, { attempts = DEFAULT_ATTEMPTS } = {}) {
	...
	for (let i = 0; i < attempts; i++) {
		try {
			...
			// Admin-only collection rejected while token absent/expired (surfaces as 403
			// "Only admins..." on this old PB) -> re-auth and retry, bounded.
			if (ADMIN_403_REAUTH && isAdminRejected(err) && adminReauths < MAX_ADMIN_REAUTH) {
				adminReauths += 1;
				try {
					await forceReauth();
				}
				catch {
					// If re-auth itself fails, surface the original error.
					throw err;
				}
				continue;
			}
