...
async function renderCharacterSheetEmbed(characterRecord) {
	...
	const embed = new EmbedBuilder()
		.setTitle(nameLine)
		.addFields(
			{ name: 'Vitals', value: `\`HP\` ${safeInline(characterRecord.hp)}/${safeInline(characterRecord.hp_max)}\n\`XP\` ${String(characterRecord.xp ?? 0)}`, inline: true },
		);

	// De-emphasize metadata: keep it, but move it to the footer.
	// Use a mention so it's still easy to see who owns the sheet.
	embed.setFooter({ text: `Owner: <@${characterRecord.owner_user_id}>` });
